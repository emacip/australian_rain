...
Se puede observar en la grafica siguiente que no ninguna variable individual es mejor que otra para diferenciar si llueve o no llueve, estudiaremes `MinTemp`, `MaxTemp`, `Temp9am`, `Temp3pm` y las `Pressure9am` y las `Pressure3pm`.

```{r grafico violin variables, echo=FALSE,  fig.align='center'}
...
Ahora crearemos unas nuevas varibles combinandolas entre si.

Primero `MinTemp` y `MaxTemp`
...
```{r grafico violin nuevas variables, echo=FALSE}
...
###Estandarización:
```{r modelo}
#TODO revisar el modelo y terminarlo.
